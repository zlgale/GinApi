...
func UserInsert(uid, username, departname, created string) bool {
	...
	//checkErr(err)
	stmt, err := models.DB.Prepare(`INSERT into userinfo (uid,username,departname,created) values (?,?,?,?)`)
	checkErr(err)
...
func UserQueryByUserName(name string) User {
	...
	rows := (models.DB).QueryRow(`SELECT * FROM userinfo where username = ?`, name)
	////普通demo
	var u User
...
func UserUpdate() {
	...
	stmt, err := models.DB.Prepare(`UPDATE user SET user_age=?,user_sex=? WHERE user_id=?`)
	checkErr(err)
...
func UserRemove() {
	...
	stmt, err := models.DB.Prepare(`DELETE FROM user WHERE user_id=?`)
	checkErr(err)
	res, err := stmt.Exec(1)
	checkErr(err)
	num, err := res.RowsAffected()
